...
class PembayaranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($pembayaran)
        {
            return redirect()->route('siswa.index')->with(['success' => 'Data Pembayaran Berhasil di Tambahkan']);
        }
        else
        {
            return redirect()->route('siswa.index')->with(['error' => 'Data Pembayaran Gagal di Tambahkan']);
        }
    }

    public function show(string $id){
        $pembayaran = Pembayaran::where('siswa_id', $id)->paginate(5);
